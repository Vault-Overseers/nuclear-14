    ...
    public sealed partial class DamageSpecifier : IEquatable<DamageSpecifier>
    {
            ...
            float ignoreCoefficients = 0f)
        {
            ...
            foreach (var (key, value) in damageSpec.DamageDict)
            {
                ...
                if (modifierSet.Coefficients.TryGetValue(key, out var coefficient))
                    newValue *= Math.Min(1f, coefficient + ignoreCoefficients); // coefficients can heal you, e.g. cauterizing bleeding
            ...
            float ignoreCoefficients = 0f)
        {
            ...
            foreach (var set in modifierSets)
            {
                ...
                newDamage = ApplyModifierSet(newDamage, set, ignoreCoefficients);
                any = true;
            }
